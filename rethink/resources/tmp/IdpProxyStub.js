...
*/var IdpProxy={ /**
  ...
  */validateAssertion:function validateAssertion(assertion,origin){return new Promise(function(resolve,reject){var i=googleInfo;var decodedContent=atob(assertion);var content=JSON.parse(decodedContent);sendHTTPRequest('GET',i.tokenInfo+content.tokenID).then(function(result){if(JSON.stringify(result)===JSON.stringify(content.tokenIDJSON)){resolve({identity:content.tokenIDJSON.email,contents:content.tokenIDJSON});}else {reject('invalid');}},function(err){reject(err);});});}, /**
...
return new Promise(function(resolve,reject){if(!contents){ /*try {
          if (window) {
            resolve('url');
          }
        } catch (error) {*/var requestUrl=i.authorisationEndpoint+'scope='+i.scope+'&client_id='+i.clientID+'&redirect_uri='+i.redirectURI+'&response_type='+i.type+'&state='+i.state+'&access_type='+i.accessType;reject({name:'IdPLoginError',loginUrl:requestUrl}); //  }
...
var accessToken=urlParser(contents,'access_token');var idToken=urlParser(contents,'id_token');var code=urlParser(contents,'code');exchangeCode(code).then(function(value){ //obtain information about the user
