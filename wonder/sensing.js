'use strict';
var sensingController =  angular.module('sensingController', ['sessionManager','contactManager', 'googlechart' ]);

sensingController.controller("sensingController", function ($scope, $location, wonderFactory) {
	console.log("BIMBO")
    var chart1 = {};
    ...
    chart1.options = {
        "title": "Smartwatch bpm",
        "isStacked": "true",
        "fill": 20,
        "displayExactValues": true,
        "vAxis": {
            "title": "bpm", "gridlines": {"count": 10}
        },
        "hAxis": {
            "title": "Date"
        }
...
$scope.verGraf = function () {
	$location.path("sensing");
}
    $scope.sendData = function () {
	console.log("ennnn-");
	setInterval(function () {
	var smart = Math.floor((Math.random() * 100) + 1);
	var newMessage = new DataMessage(codec.id, "", wonderFactory.getMyIdentity().rtcIdentity, smart, MessageType.MESSAGE);
        codec.send(JSON.stringify(newMessage));
	}, 3000);
	var newMessage = new DataMessage(codec.id, "", wonderFactory.getMyIdentity().rtcIdentity, "100", MessageType.MESSAGE);
        codec.send(JSON.stringify(newMessage));
